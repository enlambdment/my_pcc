...
class Ship():
	...
	def __init__(self, ss_settings, screen):
		...
		# This time we are able to move *up or down.*
		self.moving_up = False
		self.moving_down = False

	def update(self):
		"""Update the ship's position based on movement flags."""
		# Instead of working directly on rect.centery,
		# we will work on the float first ...
		if self.moving_up and self.rect.top > 0:
			self.center -= self.ss_settings.ship_speed_factor
		if self.moving_down and self.rect.bottom < self.screen_rect.bottom:
			self.center += self.ss_settings.ship_speed_factor

		# ... then update rect object from self.center.
